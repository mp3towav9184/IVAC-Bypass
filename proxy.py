...
import subprocess, atexit, socket, json, random, requests
from string import ascii_letters, digits

SESSION = ''.join(random.choices(ascii_letters + digits, k=8))

USER = f'rmdo6ddrr8z1z5x-country-bd-session-{SESSION}-lifetime-120'
PASS = f'3t6chpshccf5qvh'
...
def request(flow: http.HTTPFlow) -> None:
    ...
    if 'page_bg' in url:
        flow.response = http.Response.make(302, b"", {"Location": "https://cellar-c2.services.clever-cloud.com/static.talan.com/s3fs-public/styles/half_banner/public/2025-09/Security-Password-Hacking.jpg.webp"})
...
def response(flow: http.HTTPFlow) -> None:
    url = flow.request.pretty_url
    if 'ivacbd.com/api/v2/' in url and flow.response.status_code == 429:
        try: retryAfter = int(flow.response.headers.get('retry-after', 0))
        except Exception as err:
            return print(f'Error ({type(err)}): {err}')
        if retryAfter > 90: return
        if retryAfter:
            print(f'[*] Waiting {retryAfter} sec before executing API')
            sleep(retryAfter + 1)
            flow.response = http.Response.make(307, b"", {"Location": url})
        
        # try:
        #     pr = f'http://{USER}:{PASS}@{HOST}:{PORT}'
...
def runProxyServer(wait=False):
    ...
    p = subprocess.Popen(['mitmdump', '-s', __file__, '--listen-port', str(PORT), '--upstream-auth',
                         f'{USER}:{PASS}', '--quiet'], stderr=subprocess.STDOUT)
